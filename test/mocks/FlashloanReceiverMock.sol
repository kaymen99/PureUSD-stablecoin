...
contract FlashloanReceiverMock is IFlashloanReceiver {
    constructor(
        address dscController,
        address dscToken,
        address weth,
        address wbtc
    ) {
        IERC20(dscToken).approve(address(dscController), type(uint256).max);
        IERC20(weth).approve(address(dscController), type(uint256).max);
        IERC20(wbtc).approve(address(dscController), type(uint256).max);
    }
