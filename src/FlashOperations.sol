...
/// @dev Will allow users to perform flash mint of DSC token or to flashloan collateral tokens kept in the controller contract.
abstract contract FlashOperations {
    // can flashmint DSC token or flashloan allowed collateral tokens
    ...
    error DSCTotalSupplyHasChanged();
    ...
    /// @param token address of flashloaned token, set to dscToken for flashMint.
    ...
    /// @dev mint DSC amount to user and burn it after flashOps ends, must provide exact fee amount
    /// @dev DSC token total supply must not change during flashlaon
    /// @param receiver contract that will receive flash ops funds
    /// @param amount amount of token desired, must be non-zero.
    /// @param params user-defined parameters.
    function _runFlashMint(
        ...
        IDSCToken token = DSCToken();
        uint256 dscSupplyBefore = token.totalSupply();
        uint256 fee = (amount * flashOpsFeeBPS) / 1e18;

        // mint DSC token amount to
        ...
        // check that DSC total supply did not change
        uint256 dscSupplyAfter = token.totalSupply();
        if (dscSupplyAfter != dscSupplyBefore)
            revert DSCTotalSupplyHasChanged();
    }
    ...
    /// @notice Function to get the DSC token
    function DSCToken() public view virtual returns (IDSCToken) {}
}
